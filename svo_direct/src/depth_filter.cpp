    ...
    const double depth_max)
{
  ...
  else
  {
    ...
    while(!jobs_.empty())
      jobs_.pop();
    jobs_.push(Job(frame, depth_min, depth_max, depth_mean)); // this put SEEDS INIT.
    jobs_condvar_.notify_all(); // who is waiting for this  Not ceres.
  }
}
  ...
  while(true)
  {
    ...
    Job job;
    {
      ...
      while(jobs_.empty() && !quit_thread_)
        jobs_condvar_.wait(lock); // this will notified when new frame 
  ...
  else
  {
    ...
    for(const FramePtr& ref_frame : ref_frames_with_seeds)
    {
      for(size_t i = 0; i < ref_frame->num_features_; ++i)
      {
        ...
        {
          jobs_.push(Job(cur_frame, ref_frame, i)); // THIS WILL PUSH UPDATE.
        }
      }
    }
    jobs_condvar_.notify_all();
