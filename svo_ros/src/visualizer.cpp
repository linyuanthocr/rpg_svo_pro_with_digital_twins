    ...
    const double alpha = 1.0)
{
  ...
  marker.id = 0;
  marker.ns = ns;
  marker.header.frame_id = svo::Visualizer::kVioFrame;
    ...
    const std::vector<float>& intensities)
{
  ...
  pc.header.frame_id = svo::Visualizer::kVioFrame;
  pc.clear();
...
std::string Visualizer::kVioFrame = std::string("vio");
...
Visualizer::Visualizer(const std::string& trace_dir,
  ...
  , publish_active_keyframes_(vk::param<bool>(pnh_, "publish_active_kfs", false))
  ...
  , vis_scale_(vk::param<double>(pnh_, "publish_marker_scale", 1.2))
{
  ...
  pub_points_ = pnh_.advertise<visualization_msgs::Marker>("points", 10000); //TODO: nothing is published here when using with flightmare
  pub_imu_pose_ = pnh_.advertise<geometry_msgs::PoseStamped>("pose_imu", 10);
  ...
  for (size_t i = 0; i < n_cameras; ++i)
  {
    ...
    pub_images_.at(i) = it.advertise("image_with_features/" + std::to_string(i), 10);
  ...
  pose_graph_map_.header.frame_id = kVioFrame;
  pub_loop_closure_ =
                                    ...
                                    const int64_t timestamp_nanoseconds)
{
  vk::output_helper::publishTfTransform(
      ...
      ros::Time().fromNSec(timestamp_nanoseconds), "cam_pos", kVioFrame, br_);
                                  ...
                                  const std::string frame_id)
{
  ...
  publishTrajectoryPoint(frame->pos(), timestamp, trace_id_, frame_id);
...
void Visualizer::publishTrajectoryPoint(const Eigen::Vector3d& pos_in_vision,
                                        const uint64_t timestamp, const int id, const std::string frame_id)
{
  if (pub_points_.getNumSubscribers() > 0)
  {
    if ( pos_in_vision.hasNaN()) {
      std::cout << "pos_in_vision has NaN" << std::endl;
    }
    ...
    vk::output_helper::publishPointMarker(
        ...
        0.5 * trajectory_marker_scale_ * vis_scale_, Vector3d(0., 0., 0.5),frame_id);
  }
}
                                          ...
                                          bool reset_pc_before_publishing)
{
  ...
  pc_->header.frame_id = kVioFrame;
  pc_->header.stamp = ros::Time::now().toNSec();
  pc_->reserve(frame.num_features_);
  std::cout << "nb points in current frame: " << frame.num_features_ << std::endl;
  for (size_t i = 0; i < frame.num_features_; ++i)
  {
...
void Visualizer::publishSeedsUncertainty(const Map::Ptr& map)
{
  ...
  msg_variance.header.frame_id = kVioFrame;
...
void Visualizer::publishMapRegion(const std::vector<FramePtr>& frames) // this is what actually publishs the converged point. Called when new keyframe is added
{
  ...
  if (pub_points_.getNumSubscribers() > 0) // this is "svo/point" -> seems like a bigger set then svo/pointcloud
  {
    for (const FramePtr& frame : frames)
      publishKeyframeWithPoints(frame, ++ts, point_marker_scale_, kVioFrame);
  }
}
                                           ...
                                           const std::string frame_id)
{
  ...
  vk::output_helper::publishFrameMarker(
      ...
      "kfs", ros::Time::now(), frame->id_ * 10, 0, marker_scale * 2.0, kVioFrame);
  ...
  for (size_t i = 0; i < frame->num_features_; ++i)
  {
    ...
    if (isEdgelet(frame->type_vec_[i]))
    {
      vk::output_helper::publishPointMarker(
          ...
          marker_scale * vis_scale_, Vector3d(0, 0.6, 0),frame_id,
    ...
    else
    {
      vk::output_helper::publishPointMarker(
          pub_points_, xyz_world, "pts", ros::Time::now(), id, 0,
          marker_scale * vis_scale_, Vector3d(1, 0, 1),frame_id,
          publish_points_display_time_);
    }
...
void Visualizer::publishActiveKeyframes(const std::vector<FramePtr>& active_kfs)
{
  ...
  for (size_t i = 0; i < active_kfs.size() - 1; i++)
  {
    vk::output_helper::publishLineMarker(
        ...
        Eigen::Vector3d(.0, .0, 0.5), svo::Visualizer::kVioFrame);
  }
}
...
void Visualizer::exportToDense(const FrameBundlePtr& frame_bundle)
{
  ...
  for (size_t cam_index = 0; cam_index < frame_bundle->size(); ++cam_index)
  {
        ...
        pub_dense_.at(cam_index).getNumSubscribers() > 0)
    {
      ...
      msg.header.frame_id = svo::Visualizer::kVioFrame;
...
void Visualizer::visualizeCoordinateFrames(const Transformation& T_world_cam)
{
  ...
  vk::output_helper::publishFrameMarker(
      ...
      "cam", ros::Time::now(), 0, 0, 0.2,kVioFrame);

  // origin frame
  vk::output_helper::publishFrameMarker(pub_markers_, Matrix3d::Identity(),
                                        Vector3d::Zero(), kVioFrame,
                                        ros::Time::now(), 0, 0, 0.2,kVioFrame);
}
                                  ...
                                  const size_t ignored_past_frames)
{
  ...
  if (pub_pose_graph_.getNumSubscribers() > 0)
  {
    ...
    pose_graph_pc.header.frame_id = kVioFrame;
    pose_graph_pc.clear();
