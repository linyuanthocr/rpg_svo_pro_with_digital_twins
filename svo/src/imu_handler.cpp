...
bool ImuHandler::getAngularVelocity(double timestamp, Eigen::Vector3d& omega) const
{
    ...
    Quaternion& R_imu_world) const
{
  ...
  VLOG(0) << "Initial Rotation = " << C_imu_world;
...
IMUTemporalStatus ImuHandler::checkTemporalStatus(const double time_sec)
{
  ...
  for (size_t idx = 0; idx < temporal_imu_window_.size(); idx++)
  {
    ...
    if (start_idx != 1 && (time_sec - temporal_imu_window_[idx].timestamp_ > options_.temporal_window_length_sec_))
    {
      end_idx = idx;
      break;
    }
