...
public:
  ...
  /// Get pose of camera rig. -> camera ? Body / imu instead no?
  Transformation get_T_W_B() const {
    CHECK(!frames_.empty());
    return frames_[0]->T_world_imu();
  }

  //TODO : whats happening here ? by doing set_T_W_B we are setting the pose of the CAMERA ?
  /// Set pose of camera rig. -> 
  void set_T_W_B(const Transformation& T_W_B) {
    int u =0;
    for(const FramePtr& frame : frames_) {
       frame->T_f_w_ = (T_W_B*frame->T_body_cam_).inverse(); // cam to world,  here is really camera ?
    }
  }
