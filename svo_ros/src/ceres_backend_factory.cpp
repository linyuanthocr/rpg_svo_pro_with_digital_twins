                                       ...
                                       const CameraBundlePtr& camera_bundle)
{
  ...
  backend_options.verbose = vk::param<bool>(pnh, "ceres_verbose", false);
  /// Apply marginalization?
  backend_options.marginalize = vk::param<bool>(pnh, "ceres_marginalize", true);
  /// Maximum number of iterations
  backend_options.num_iterations =  static_cast<size_t>(vk::param<int>(pnh, "ceres_num_iterations", 3));
  backend_options.num_threads =  static_cast<size_t>(vk::param<int>(pnh, "ceres_num_threads", 2));
  /// Sliding window size of ceres backend
  backend_options.num_imu_frames =  static_cast<size_t>(vk::param<int>(pnh, "ceres_num_imu_frames", 3));
  ...
  /// maximum number of iterations.
  backend_options.max_iteration_time = static_cast<double>(vk::param<double>(pnh, "ceres_max_iteration_time", -1.0));
  ...
  CeresBackendInterface::Ptr ba_interface = std::make_shared<CeresBackendInterface>(ba_interface_options,
                                              backend_options,
