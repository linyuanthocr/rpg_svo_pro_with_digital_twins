                   ...
                   const double& t_0, const double& t_1)
{
  ...
  DEBUG_CHECK(imu_measurements.front().timestamp_ >= gp_measurements.front().timestamp_)
      << "Newest IMU measurement is older than newest gp measurement! : imu : " << imu_measurements.front().timestamp_  << " gps : " << gp_measurements.front().timestamp_ ;



  DEBUG_CHECK(imu_measurements.back().timestamp_ < gp_measurements.back().timestamp_)
      << "Oldest IMU measurement is newer than oldest gp measurement! : imu : " << imu_measurements.back().timestamp_  << " gps : " << gp_measurements.back().timestamp_ ;
...
{ 
  ...
  // Newest imu meas newer than newest gp meas.->happens in simulation because tey are exactly the same timestammp.
                                            ...
                                            double** jacobians_minimal) const
{
  ...
  // actual propagation output:
  {
    ...
    for (size_t k = 0; k < n_residuals; ++k)
    {
      ...
      const Eigen::Quaterniond Dq = deltaQ(-dalpha_db_g_vec_[k]*Delta_b.head<3>())*Delta_q_vec_[k];
      ...
      Eigen::Matrix<double,3,15> F0 =  Eigen::Matrix<double,3,15>::Identity(); // holds for d/db_g, d/db_a
