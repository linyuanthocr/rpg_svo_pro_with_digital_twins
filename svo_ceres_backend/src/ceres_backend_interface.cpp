    ...
    const Map::Ptr& map, bool& have_motion_prior)
{
  ...
  if (addStatesInertialGpAndGposeMeasurementsToBackend(new_frames))
  {
  ...
  {
    IMUTemporalStatus imu_status = imu_handler_->checkTemporalStatus(new_frames->at(0)->getTimestampSec());
    imu_motion_detector_stationary_ = (imu_status == IMUTemporalStatus::kStationary);
  }
...
void CeresBackendInterface::setT_w_vio(const Transformation T_w_vio, bool refined) {
  T_w_vio_ = T_w_vio;
  T_vio_w_ = T_w_vio.inverse();
  backend_.setT_w_vio(T_w_vio);
  publisher_->set_T_w_vio(T_w_vio);
  if(refined) 
    publisher_->update_refinement_state(true);
}

void CeresBackendInterface::setT_w_vio_temp(const Transformation T_world_vio_first_estimate) {
  publisher_->set_T_w_vio_temp(T_world_vio_first_estimate);
}
...
// Introduce a state for the frame_bundle in backend. Add IMU and Gp terms.
bool CeresBackendInterface::addStatesInertialAndGpMeasurementsToBackend(
    const FrameBundlePtr& new_frame_bundle)
{
  // Gather required IMU measurements ----------------------------------------
  ImuMeasurements imu_measurements;
  const double current_frame_bundle_stamp =  new_frame_bundle->getMinTimestampSeconds();
                                                    ...
                                                    imu_measurements, true))
  {
    LOG(ERROR) << "Could not retrieve IMU measurements."
               << " Last frame was at " << last_added_frame_stamp_ns_
               << ", current is at "
               << new_frame_bundle->getMinTimestampNanoseconds();
    return false;
  }
  ...
  double current_frame_bundle_stamp_adjusted = current_frame_bundle_stamp - imu_handler_->imu_calib_.delay_imu_cam;
  double oldest_timestamp = imu_measurements.at(imu_measurements.size()-2).timestamp_;

  if (use_global_measurements_)
  {
    if (!globalpositions_handler_->getMeasurementTillTime(current_frame_bundle_stamp_adjusted, oldest_timestamp, gp_measurement, true))
    {
      if (!backend_.addStates(new_frame_bundle, imu_measurements, gp_measurement,
                          current_frame_bundle_stamp)) //TODO : why not usng the adjusted one ???
      {
        LOG(ERROR) << "Failed to add state. Will drop frames.";
        return false;
      }
    ...
    else 
    {
      ...
      if (gp_measurement.timestamp_ > 0.0)
      {
        ...
        if ((g_permon_gp_))
        {
          double dt_frame_first_rts_meas = gp_measurement.timestamp_ -
                  last_added_frame_stamp_ns_*1e-9;
          g_permon_gp_->log("dt_frame_first_rts_meas", dt_frame_first_rts_meas);
          g_permon_gp_->writeToFile();
        }
      }
...
bool CeresBackendInterface::addStatesInertialGpAndGposeMeasurementsToBackend(const FrameBundlePtr& new_frame_bundle){
  ...
  // new_frame_bundle
  if (!imu_handler_->getMeasurementsContainingEdges(current_frame_bundle_stamp,
                                                    imu_measurements, true)) // Here is not deterministic at the beginning
  {
    LOG(ERROR) << "Could not retrieve IMU measurements."
               << " Last frame was at " << last_added_frame_stamp_ns_
               << ", current is at "
  ...
  //Get global pose measurement ---------------------------------------------
  GposeMeasurement gpose_measurement;
  gpose_measurement.timestamp_ = -1.0;
  gpose_measurement.position_ = Eigen::Vector3d::Zero();
  gpose_measurement.orientation_ = Eigen::Quaterniond::Identity();

  double current_frame_bundle_stamp_adjusted = current_frame_bundle_stamp - imu_handler_->imu_calib_.delay_imu_cam;
  double oldest_timestamp = imu_measurements.at(imu_measurements.size()-2).timestamp_; //-2 hard coded because imu window is 3??? check and fix this


  if ((use_global_measurements_) && (use_map_registration_)) // CAse 1
  {
    /**We can have : 
     * - No gps + no map_registrated_pose MR - case A
     * - No gps + map_registrated_pose - case B. This one still need the gps to be initialized.
     * - gps but no map_registrated_pose - case C
     * - gps and map_registrated_pose - case D 
    **/

    double t_last_kf = 0.0;
    if (last_added_nframe_imu_ != 0)
    {
      auto id_last_kf = backend_.currentKeyframeId(); 
     
      t_last_kf = backend_.OldestKFStamp() ; 
    }

    if (!globalpositions_handler_->getMeasurementTillTime(current_frame_bundle_stamp_adjusted, oldest_timestamp, gp_measurement, true)) {
      if (!globalpose_handler_->getMeasurementTillTime(current_frame_bundle_stamp_adjusted, t_last_kf/*oldest_timestamp*/, gpose_measurement, true)) { // case A

        // Neither gps or map_registrated_pose
        if (!backend_.addStates(new_frame_bundle, imu_measurements, gp_measurement, current_frame_bundle_stamp)) {
          LOG(ERROR) << "Failed to add state. Will drop frames.";
          return false;
        }
      ...
      else { // No gps but map_registrated_pose. case B
        if(true) { // replace by if gps initalized or so.
          if (!backend_.addStates(new_frame_bundle, imu_measurements, gpose_measurement, current_frame_bundle_stamp)) { //if (!backend_.addStates(new_frame_bundle, imu_measurements, gpose_measurement, current_frame_bundle_stamp))
            LOG(ERROR) << "Failed to add state. Will drop frames.";
            return false;
          }
        }
      ...
      //gps but no map_registrated_pose
      if (!globalpose_handler_->getMeasurementTillTime(current_frame_bundle_stamp_adjusted, t_last_kf/*oldest_timestamp*/, gpose_measurement, true)) { // case C

        if (!backend_.addStates(new_frame_bundle, imu_measurements, gp_measurement, current_frame_bundle_stamp)) {
          LOG(ERROR) << "Failed to add state. Will drop frames.";
          return false;
        }
      } else { // gps and map_registrated_pose - case D

        if (!backend_.addStates(new_frame_bundle, imu_measurements, gp_measurement, gpose_measurement,
                              current_frame_bundle_stamp)) {
          LOG(ERROR) << "Failed to add state. Will drop frames.";
          return false;
        }
  ...
  else if ((use_global_measurements_) && (!use_map_registration_)) // Same as in SVO GPS. - case 2 
  {
    if (!globalpositions_handler_->getMeasurementTillTime(current_frame_bundle_stamp_adjusted, oldest_timestamp, gp_measurement, true)) {
...
// Performance monitor for benchmarking, ict project related
void CeresBackendInterface::setPerformanceMonitorGp(const std::string& trace_dir)
{
  // Initialize Performance Monitor
  g_permon_gp_.reset(new vk::PerformanceMonitor());
  g_permon_gp_->addLog("dt_frame_first_gp_meas");
  g_permon_gp_->init("trace_gp_meas", trace_dir);
}

void CeresBackendInterface::optimizationLoop()
{
  bool last_frame_was_keyframe = false;
  ...
  {
    {
...

      {
        ...
        if (is_w_T_valid_) // in case of loop closing 
        {
      ...
      if (g_permon_backend_) // log marginalization
      {
      ...
      {
        backend_.setAllFixedLandmarksEnabled(false); // disable fixed landmark reprojection error
        if (!backend_.hasFixedPose())
        {
          backend_.setOldestFrameFixed(); // fix the oldest frame, avoid drifting 
        }
      ...
      // Log Ceres Map ----------------------------------------------------------
      // Only partial (about mainly inertial part) logging.
      VLOG(1) << "--- Backend: Inertial Map ---";
      ...
      size_t nParamBlocks = 0;
      for(auto& p: idToParameterBlockMap)
      {
        if( (p.second->typeInfo()=="PoseParameterBlock") ||
              (p.second->typeInfo()=="SpeedAndBiasParameterBlock") )
        {
          nParamBlocks++;
          //curr_map->printParameterBlockInfo(p.first);
        }
      }
      VLOG(1) << "Map containes " << nParamBlocks << " inertial parameter blocks.";

      // Residuals
      std::unordered_map<ceres::ResidualBlockId, ceres_backend::Map::ResidualBlockSpec> residualBlockIdToResidualBlockSpecMap;
      ...
      std::vector<ceres_backend::Map::ParameterBlockSpec> param_block_collection;
      ...
      switch(r.second.error_interface_ptr->typeInfo())
      {
        ...
        case ceres_backend::ErrorType::kGpError : nGpError++; break;
      }
      }
      size_t nResiduals = nHomogeneousPointError + nReprojectionError + nSpeedAndBiasError +
            nMarginalizationError + nPoseError + nIMUError + nRelativePoseError + nGpError;
      VLOG(0) << "Map containes " << nResiduals << " residuals:";
      VLOG(0) << " - nHomogeneousPointError " << nHomogeneousPointError;
      VLOG(0) << " - nReprojectionError " << nReprojectionError;
      VLOG(0) << " - nSpeedAndBiasError " << nSpeedAndBiasError;
      VLOG(0) << " - nMarginalizationError " << nMarginalizationError;
      VLOG(0) << " - nPoseError " << nPoseError;
      VLOG(0) << " - nIMUError " << nIMUError;
      VLOG(0) << " - nRelativePoseError " << nRelativePoseError;
      VLOG(0) << " - nGpError " << nGpError;

      VLOG(0) << "Num states connected to global position residuals : "
      ...
      {
      last_frame_was_keyframe = active_keyframes_.back()->bundle_id_ == last_optimized_nframe_.load();

     
        backend_.get_T_WS(active_keyframes_.back()->bundle_id_, T_vio_b_last_kf_);
      ...
      last_state_.setTimestamp(last_added_frame_stamp_ns_);

      // publish current estimation
      if (publisher_)
      {
        publisher_->publish(last_state_, last_added_frame_stamp_ns_,
                            last_optimized_nframe_.load(), last_frame_was_keyframe);
      }
...
{
  globalpositions_handler_ = globalpositions_handler;

  GpParameters rts_parameters;
  rts_parameters.cov = globalpositions_handler_->getCovariance();
  rts_parameters.B_t_PB = globalpositions_handler_->getBodyPrismDistance();
  rts_parameters.max_num_residuals = globalpositions_handler_->gp_settings_.max_num_residuals;
  rts_parameters.initial_orientation_known =
          globalpositions_handler_->gp_settings_.initial_orientation_known;
  backend_.addGpParameters(rts_parameters);
}

// Set the global position settings in backend
void CeresBackendInterface::setGposeHandler(
  const std::shared_ptr<GlobalPoseHandler> globalpose_handler)
{
  globalpose_handler_ = globalpose_handler;

  GposeParameters rts_parameters;
  rts_parameters.cov = globalpose_handler_->getCovariance();

  rts_parameters.B_t_PB = globalpose_handler_->getBodyPrismDistance();
  rts_parameters.B_q_PB = globalpose_handler_->getBodyWorldRotation();

  rts_parameters.max_num_residuals = globalpose_handler_->gp_settings_.max_num_residuals;
  rts_parameters.initial_orientation_known =  globalpose_handler_->gp_settings_.initial_orientation_known;
  backend_.addGposeParameters(rts_parameters); // DO THIS
}
