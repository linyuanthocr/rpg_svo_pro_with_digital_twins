...
bool AbstractInitialization::trackFeaturesAndCheckDisparity(const FrameBundlePtr& frames)
{
  ...
  // disparity = difference in coordinates of similar features within two or more different images.
  tracker_->trackFrameBundle(frames);

  std::vector<size_t> num_tracked;
  std::vector<double> disparity;

  tracker_->getNumTrackedAndDisparityPerFrame(options_.init_disparity_pivot_ratio, &num_tracked, &disparity);

  const size_t num_tracked_tot =  std::accumulate(num_tracked.begin(), num_tracked.end(), 0u);
  const double avg_disparity = std::accumulate(disparity.begin(), disparity.end(), 0.0) / disparity.size();
...
if(SVO_USE_OPENGV) {
  ...
  // Create vector of bearing vectors
  const FrameBundlePtr frames_ref = tracker_->getOldestFrameInTrack(0);
  FeatureMatches matches_cur_ref; // contains all the matches features (300 or so...)
  feature_tracking_utils::getFeatureMatches( *frames_cur->at(0), *frames_ref->at(0), &matches_cur_ref);

  // Create vector of bearing vectors
  BearingVectors f_cur;
  BearingVectors f_ref;
  initialization_utils::copyBearingVectors(*frames_cur->at(0), *frames_ref->at(0), matches_cur_ref, &f_cur, &f_ref);
  ...
  // Print all bearing vectors in f_cur
for (size_t i = 0; i < 10; ++i) {
    Eigen::Vector3d bearing_vector = f_cur[i];
}

// Print all bearing vectors in f_ref
for (size_t i = 0; i < 10; ++i) {
    Eigen::Vector3d bearing_vector = f_ref[i];
}

  opengv::relative_pose::CentralRelativeAdapter adapter(f_cur, f_ref);
  std::shared_ptr<CentralRelative> problem_ptr(new CentralRelative(adapter, CentralRelative::NISTER));

  // Assuming CentralRelative provides methods to access model parameters
  Eigen::Matrix3d rotation_ = adapter.getR12(); // Get the estimated rotation_ matrix
  Eigen::Vector3d translation_ = adapter.gett12(); // Get the estimated translation_ vector

  opengv::sac::Ransac<CentralRelative> ransac;
  ransac.sac_model_ = problem_ptr;
  ransac.threshold_ = inlier_threshold; // ?
  ransac.max_iterations_ = 1000; //100;
  ransac.probability_ =  0.995; // 0.5 
  ransac.computeModel();

  // NOTE: so when we finally find a goos enough disparity, we arrive at "0 inliers and fail here, leading to reinitialization.
  if(ransac.inliers_.size() < options_.init_min_inliers)
  {
    // Access the computed model (rotation and translation)
    Eigen::Matrix3d rotation = ransac.model_coefficients_.block<3, 3>(0, 0);
    Eigen::Vector3d translation = ransac.model_coefficients_.block<3, 1>(0, 3);
