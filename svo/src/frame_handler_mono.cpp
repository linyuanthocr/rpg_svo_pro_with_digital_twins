...
UpdateResult FrameHandlerMono::processFirstFrame()
{
  ...
  const auto res = initializer_->addFrameBundle(new_frames_); //going to frame_hander_base.cpp

  if(res == InitResult::kTracking) {
    // we stay here for the flightmare case....
  ...
  stage_ = Stage::kTracking; // switch to tracking mode -> we never arrive there
...
UpdateResult FrameHandlerMono::processFrame()
{
  ...
  // redundant when using ceres backend ->is here where we are f up ?
  //TODO: check here 
  ...
  frame_utils::getSceneDepth(newFrame(), depth_median_, depth_min_, depth_max_); // HERE IS THE ONE ACTUALLY USED -> not entered in the loop with flightmare
  VLOG(40) << "Current Frame Depth: " << "min: " << depth_min_
           << ", max: " << depth_max_ << ", median: " << depth_median_;


  // Untile here seems to be working ok
...
//      depth_filter_->updateSeeds(overlap_kfs_.at(0), newFrame());
  ...
  // Question : where do we set the new keyframe for ceres backend ?
