...
bool FrameHandlerBase::addImageBundle(const std::vector<cv::Mat>& imgs, const uint64_t timestamp)
{
  ...
  {
    frames.push_back(std::make_shared < Frame > (cams_->getCameraShared(i), imgs[i].clone(), timestamp, options_.img_align_max_level + 1));
...
void FrameHandlerBase::setInitialPose(const FrameBundlePtr& frame_bundle) const
{
  ...
  else // default situation, gps or not. the difference lies in T_world_imuinit_ which would be modified by the gps if used. 
  //Keep it like this : x y z gps t0 = x y z VIO t0.  
  {
    VLOG(40) << "Set initial frame pose.";
    for (size_t i = 0; i < frame_bundle->size(); ++i)
    { //pose world base from frame of cam 0 and cam 1 
      frame_bundle->at(i)->T_f_w_ = cams_->get_T_C_B(i) * T_world_imuinit_.inverse(); // frame from world = T_cam_body*T_body_world = T_cam_world
      VLOG(40) << "Frame " << i << ":" << std::endl << frame_bundle->at(i)->T_f_w_;
    }

    frame_bundle->at(0)->set_T_cam_imu(cams_->get_T_C_B(0));
...
size_t FrameHandlerBase::sparseImageAlignment()
{
  ...
  sparse_img_align_->setMaxNumFeaturesToAlign(1000); //options_.img_align_max_num_features);

  size_t img_align_n_tracked = sparse_img_align_->run(last_frames_, new_frames_); // here the prior is weird
