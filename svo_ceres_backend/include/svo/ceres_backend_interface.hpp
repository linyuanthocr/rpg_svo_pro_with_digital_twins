...
public:
  ...
  /**
   * @brief Set the Robotic total station in backend
   * @param globalpositions_handler Shared RtsHandler with frontend
   */
  void setGpHandler(const std::shared_ptr<GlobalPositionsHandler> globalpositions_handler);

  void setT_w_vio(const Transformation T_w_vio, bool refined = false);

  void setT_w_vio_temp(const Transformation T_w_vio_temp);

  /**
   * @brief Set the map registration measurements in backend
   * @param globalpose_handler Shared RtsHandler with frontend
   */
  void setGposeHandler(const std::shared_ptr<GlobalPoseHandler> globalpose_handler);
  ...
  /**
   * @brief Initialize a performance monitor for global measurements
   * @param trace_dir[in] path to folder where data should be dumped
   */
  void setPerformanceMonitorGp(const std::string& trace_dir) override;
  ...
  {
    return last_optimized_nframe_.load(); // is this the last KF???
  }

  inline void getLastPoseAtKF(Transformation* T_w_b) 
  {
    *T_w_b = T_vio_b_last_kf_;
  }

  // get last pose at KF
  inline bool getPoseAtKFatTimestamp(const double timestamp, Transformation* T_w_b)
  {

    BackendId id = backend_.KFIdAtStamp(timestamp);

    if(backend_.get_T_WS(id, *T_w_b))
      return true;
...
protected:
  ...
  /**
   * @brief Introduce state for frame_bundle in backend, and add the
   *        corresponding IMU and global position measurements.
   *        This function is used as alternative to
   *        addStatesAndInertialMeasurementsToBackend().
   * @param new_frame_bundle to be added
   * @param last_frame_bundle previous frame bundle
   * @return true if successful
   */
  bool addStatesInertialAndGpMeasurementsToBackend(const FrameBundlePtr& new_frame_bundle);


  /**
   * @brief Introduce state for frame_bundle in backend, and add the
   *        corresponding IMU and global pose measurements.
   *        This function is used as alternative to
   *        addStatesAndInertialMeasurementsToBackend().
   * @param new_frame_bundle to be added
   * @param last_frame_bundle previous frame bundle
   * @return true if successful
   */
  bool addStatesInertialGpAndGposeMeasurementsToBackend(const FrameBundlePtr& new_frame_bundle);
  ...
  // transform world to VIO
  Transformation T_w_vio_;
  Transformation T_vio_w_;

  Transformation T_vio_b_last_kf_;
  ...
  // 
  PerformanceMonitorPtr g_permon_gp_;
