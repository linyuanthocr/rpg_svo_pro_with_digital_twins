    ...
    const FrameBundle::Ptr& cur_frames)
{
  ...
  ref_frames_ = ref_frames; // frame from previous optimization
  ...
  Transformation T_icur_iref = cur_frames_->at(0)->T_imu_world() * T_iref_world_.inverse(); // T_imu_ref ->isn't this 0 ?
  ...
  {
    f->T_f_w_ = f->T_cam_imu()*state.T_icur_iref*T_iref_world_; // frame to world pose
