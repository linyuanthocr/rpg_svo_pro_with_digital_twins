...
public:
  ...
  inline Transformation getLatestT_WS() const { return T_WS_backend_latest_; }

  inline Eigen::Matrix<double, 3, 1> getLatestSpeed() const { return speed_bias_backend_latest_.block<3,1>(0,0); }

  inline Eigen::Matrix<double, 6, 1> getLatestBias() const { return speed_bias_backend_latest_.block<6,1>(2,0); }

  inline double getLatestTimestamp() const { return timestamp_backend_latest_; }
  ...
  {
    T_world_imuinit_ = T_world_cam*cams_->get_T_C_B(cam_index);
  }
  ...
  /// Initial orientation
  Transformation T_world_imuinit_;
  ...
  GlobalPoseHandlerPtr globalpose_handler_;
